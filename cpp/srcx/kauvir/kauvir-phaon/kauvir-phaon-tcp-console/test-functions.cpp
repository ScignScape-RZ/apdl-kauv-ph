...
//  caon_ptr<KCM_Channel_Group> kcg = table.find_channel_group(g1);

//    //table.init_phaon_function(g1, "test-0-ss", 700, &test_0_ss);
//  Phaon_Function phf(&test_0_ss);

//  phf.augment(kcg);

//  pss["test_0_ss"].push_back(phf);
...
//                              (&test_0_ss));
...

 {
  ...
  Phaon_Function phf(&test_0_s);

  phf.augment(kcg);

  pss["test_0_s"].push_back(phf);
...

 {
  ...
  Phaon_Function phf(&test_s_ss);

  phf.augment(kcg);

  pss["test_s_ss"].push_back(phf);
